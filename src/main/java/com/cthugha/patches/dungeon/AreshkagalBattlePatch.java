	...
	public static class QueueMonstersPatch {
		...
		public static void Postfix(MonsterGroup _inst) {
			if (AbstractDungeon.getMonsters().monsters.stream()
					.anyMatch(m -> m instanceof Areshkagal && m.intent == AbstractMonster.Intent.MAGIC))
				AbstractDungeon.actionManager.monsterQueue.removeAll(
						AbstractDungeon.actionManager.monsterQueue.stream()
								.filter(i -> i.monster instanceof BurialWatchdogSword ||
										i.monster instanceof BurialWatchdogScepter)
