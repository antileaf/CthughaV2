package com.cthugha.patches.flare;
...
public class FlareMechanicsPatch {
	...
	@SpirePatch(clz = AbstractPlayer.class, method = "applyStartOfTurnCards", paramtypez = {})
	public static class StartOfTurnClearExhaustPilePatch {
		@SpirePostfixPatch
		public static void Postfix(AbstractPlayer _inst) {
			_inst.exhaustPile.group.stream()
					.filter(c -> c instanceof AbstractCthughaCard)
					.forEach(c -> ((AbstractCthughaCard) c).clearShunRanAtStartOfTurn());
		}
	}
}
