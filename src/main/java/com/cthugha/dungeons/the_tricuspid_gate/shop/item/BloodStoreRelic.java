...
public class BloodStoreRelic extends StoreRelic {
	...
	public void purchaseRelic() {
		...
		if (AbstractDungeon.player.maxHealth > this.price) {
			...
			if (shopScreen instanceof BloodShopScreen) {
				BloodShopScreen b = (BloodShopScreen) shopScreen;

				for (int page = 0; page < BloodShopScreen.PAGE_COUNT; page++)
					if (page != b.currentPage) {
						b.coloredCardsPages.get(page).forEach(c -> this.relic.onPreviewObtainCard(c));
						b.colorlessCardsPages.get(page).forEach(c -> this.relic.onPreviewObtainCard(c));
					}
			}
